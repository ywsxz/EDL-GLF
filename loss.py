...
def edl_digamma_loss(alpha, target, epoch_num, num_classes, annealing_step, device, sample_weights):
    # target = F.one_hot(target, num_classes)
    loss = sample_weights*edl_loss(torch.digamma, target, alpha, epoch_num, num_classes, annealing_step, device)
    return torch.mean(loss)
...
def get_loss(evidences, evidence_a, target, epoch_num, num_classes, annealing_step, gamma, device, w_hard, w_unc):
    target_indices = target.clone()
    target_onehot = F.one_hot(target, num_classes)
    
    alpha_a = evidence_a + 1
    loss_acc = edl_digamma_loss(alpha_a, target_onehot, epoch_num, num_classes, annealing_step, device, sample_weights=1)
    
    for v in range(len(evidences)):
        alpha = evidences[v] + 1
        
        mining_weights = get_mining_weights(evidences[v], target_indices, num_classes, w_hard, w_unc)
        
        loss_acc += edl_digamma_loss(alpha, target_onehot, epoch_num, num_classes, annealing_step, device, sample_weights=mining_weights)
...
def get_mining_weights(evidence, target, num_classes, w_hard, w_unc):
    alpha = evidence + 1
    S = torch.sum(alpha, dim=1, keepdim=True)
    ...
    if target.dim() == 1:
        target_indices = target.view(-1, 1).long() # [Batch, 1]
    else:
   
        target_indices = torch.argmax(target, dim=1).view(-1, 1)


    prob_gt = probs.gather(1, target_indices) # [Batch, 1]
    
    weight_hard_mining = (1 - uncertainty) * (1 - prob_gt)
    
    weight_uncertainty = uncertainty
    
    weights = 1.0 + (w_hard * weight_hard_mining) + (w_unc * weight_uncertainty)
